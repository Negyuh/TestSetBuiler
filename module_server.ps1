...
$Interfacenaam='supportgroups'
    ...
    for($i=0; $i -lt $update; $i++){
        $editline= ('KPN,dept,KPN-OPS BO BITO-BA ERP-Team 1,KPN-OPS BO BITO-BA ERP-Team 1,KPN-OPS BO BITO-BA ERP-Team 1')
            $edline=$editline -replace 'sysid',$hash1.sid[$i] -replace 'dept',$hash1.altdepartment[$i]  -replace 'name',$hash1.devicename[$i] -replace 'dbname',$hash1.cidbname[$i] -replace 'database',$hash1.cidbmodel[$i] -replace 'datum',$logdate -replace 'serial', $hash1.Serial[$i] -replace 'ipaddr', $hash1.ipaddress[$i] -replace 'assettag', $hash1.assettag[$i] -replace 'ciactive', $hash1.altciactive[$i] -replace 'powerstate', $hash1.altpowerstate[$i] -replace 'lastsee', $hash1.altlastseen[$i]  -replace 'ram',$hash1.altciram[$i]  -replace 'cpucore',$hash1.altcicpu[$i] -replace 'vcpu',$hash1.altcivcpu[$i]
            testfilewrite "$edline" "$Interfacenaam"}

        $edline=$editline -replace 'sysid',$hash1.sid[$i] -replace 'dept',$hash1.department[$i] -replace 'name',$hash1.devicename[$i] -replace 'dbname',$hash1.cidbname[$i] -replace 'datum',$altdatelong -replace 'serial', $hash1.Serial[$i] -replace 'ipaddr', $hash1.ipaddress[$i] -replace 'assettag', $hash1.assettag[$i] -replace 'ciactive', $hash1.ciactive[$i] -replace 'powerstate', $hash1.powerstate[$i] -replace 'lastsee', $hash1.lastseen[$i]  -replace 'ram',$hash1.ciram[$i]  -replace 'cpucore',$hash1.cicpu[$i] -replace 'vcpu',$hash1.civcpu[$i]
        testfilewrite "$edline" "$Interfacenaam"  
       $progressbar.PerformStep()
       LogWrite('Finished creation of testset for interface database part: supportgroups')}
    
#-------------------------------------------------------------------------------------------------------------------
Function sap-versions{
LogWrite('Starting creation of testset for interface database part: sap-version')
$Interfacenaam='sap-versions'
    $firstline= ('Company,Name,Version,Release')
    testfilewrite $firstline $Interfacenaam
    for($i=0; $i -lt $amountall; $i++){
        $editline= ('company,company_name_18,,')
        if($hash1.altcompany[$i] -ne $null){
            $edline=$editline  -replace 'company',$hash1.altcompany[$i] -replace 'dbrelease',$hash1.altdbrelease[$i] -replace 'name',$hash1.devicename[$i]
            testfilewrite "$edline" "$Interfacenaam"}
        else{$edline=$editline -replace 'company',$hash1.company[$i] -replace 'dbrelease',$hash1.dbrelease[$i] -replace 'name',$hash1.devicename[$i]
             testfilewrite "$edline" "$Interfacenaam"}   
       }
       $progressbar.PerformStep()
       LogWrite('Finished creation of testset for interface database part: sap-version')
}
